...
class saveTaskWindow(QtCGMaya.QDialog):
    ...
    def setup_ui(self):
        ...
        self.layout.addWidget(self.taskText, 1, 1, 1, 6)
        ...
        if stage == u'布局' or stage == u'动画':
            ...
            self.layout.addWidget(self.stereoCheckBox, 0, 2, 1, 1)
        else:
            self.layout.addWidget(self.radiobutton1, 0, 0, 1, 1)
            self.layout.addWidget(self.radiobutton2, 0, 1, 1, 1)
        self.row_Hbox.setLayout(self.layout)
        ...
        self.fileListWidget = QtCGMaya.QListWidget(self)
        ...
        self.layout4.addWidget(self.fileListWidget, 1, 0, 1, 5)
        ...
        self.fileListWidget.connect(self.fileListWidget, QtCore.SIGNAL("itemClicked (QListWidgetItem*)"),
                                    self.onClickItem)
        self.fileListWidget.connect(self.fileListWidget,
    ...
    def onSave(self, flag=False):
        ...
        #         return False
        stage = CGMaya_config.currentTask['stage']
        projectDir = CGMaya_common.makeDir(CGMaya_config.storageDir, + CGMaya_config.projectName)
        taskDir = CGMaya_common.makeDir(projectDir, CGMaya_config.currentTask['name'])

        if stage == u'布局' or stage == u'动画':
            self.stereoFlag = self.stereoCheckBox.isChecked()
        ...
        if stage == u'布局' or stage == u'动画':
            ...
            ret = self.saveAniTask(scene_fn, self.noteText.text(), self.frameText.text(), type)
        elif stage == u'灯光':
            CGMaya_config.logger.info('Saveing LightTask...\r')
            lightDir = CGMaya_common.makeDir(taskDir, CGMaya_config.CGMaya_Light_Dir)
            self.scene_fn = lightDir + '/' + self.scene_fn
            ret = self.saveLightTask(scene_fn, self.noteText.text(), singleFrame, self.frameText.text(), type)
        else:
        ...
        if stage == u'布局':
            ...
            ret = self.service.getShotInfo(project['name'], nextShotName)
            if ret == None:
                executor = ''
            else:
                try:
            ...
            if ret == None or executor == CGMaya_config.currentTask['executor']:
                ...
                if not response == reply:
                    self.service.createNextShot(project['name'], shotName, nextShotName)
    ...
    def processFile(self, filePath, zf, zipFileList):
        if os.path.isfile(filePath) and not filePath in zipFileList:
            ...
            zf.write(filePath, os.path.base(filePath))
    ...
    def saveAssetTask(self, scene_fn, note, type):
        ...
        self.service.myWFSetTaskAsset(taskID, scene_fn, projectName, '', fileID, textureFileID, [], note, '', '')
        return True
    ...
    def saveAniTask(self, scene_fn, note, frameListStr, type):
        CGMaya_config.logger.set("saveTask")
        projectName = CGMaya_config.currentTask['projectName']
        ...
        CGMaya_config.logger.info('Uploading Task File...\r')
        fileID = self.service.putFile(CGMaya_config.currentTask, scene_fn)
        self.service.etTaskShot(taskID, sceneName, projectName, '', fileID, '', audioFileIDList, note, '', frameListStr)
        refAssetIDList = CGMaya_config.currentTask['refAssetIDList']
        if refAssetIDList:
            self.service.setTaskRefAssetIDList(taskID, refAssetIDList)
            self.service.setShotRefAssetIDList(sceneName, projectName, refAssetIDList)
        return True
    ...
    def saveLightTask(self, scene_fn, note, singleFrame, frameListStr, type):
        projectName = CGMaya_config.currentTask['projectName']
        sceneName = CGMaya_config.currentTask['name']
        taskID = CGMaya_config.currentTask['_id']
        projectDir = CGMaya_common.makeDir(CGMaya_config.storageDir, projectName)
